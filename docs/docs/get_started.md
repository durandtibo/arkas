...
To make the package as slim as possible, only the packages required to use `arkas` are installed.
...
```shell
pip install arkas numpy
```

## Installing from source

To install `arkas` from source, you can follow the steps below. First, you will need to
...
```shell
git clone git@github.com:durandtibo/arkas.git
```
...
```shell
conda activate arkas
```

This example uses `conda` to create a virtual environment, but you can use other tools or
configurations. Then, you should install the required package to use `arkas` with the following
command:
