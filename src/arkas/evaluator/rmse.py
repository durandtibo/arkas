r"""Contain the root mean squared error (RMSE) evaluator."""

from __future__ import annotations

__all__ = ["RootMeanSquaredErrorEvaluator"]
...
from arkas.result import Result, RootMeanSquaredErrorResult
...
class RootMeanSquaredErrorEvaluator(BaseLazyEvaluator[RootMeanSquaredErrorResult]):
    r"""Implement the root mean squared error (RMSE) evaluator.
    ...
    def evaluate(
        ...
        logger.info(
            f"Evaluating the root mean squared error | y_true={self._y_true} | "
            f"y_pred={self._y_pred} | drop_nulls={self._drop_nulls} | "
            f"nan_policy={self._nan_policy}"
        )
        return self._evaluate(data, lazy)

    def _compute_result(self, data: pl.DataFrame) -> RootMeanSquaredErrorResult:
        return RootMeanSquaredErrorResult(
