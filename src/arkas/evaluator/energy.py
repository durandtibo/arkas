r"""Contain the energy distance between two 1D distributions
evaluator."""

from __future__ import annotations

__all__ = ["EnergyDistanceEvaluator"]
...
from arkas.result import EnergyDistanceResult, Result
...
class EnergyDistanceEvaluator(BaseLazyEvaluator[EnergyDistanceResult]):
    ...
    def evaluate(self, data: pl.DataFrame, lazy: bool = True) -> EnergyDistanceResult | Result:
        logger.info(
            f"Evaluating the energy distance | u_values={self._u_values} | "
            f"v_values={self._v_values} | drop_nulls={self._drop_nulls} | "
            f"nan_policy={self._nan_policy}"
        )
        return self._evaluate(data, lazy)

    def _compute_result(self, data: pl.DataFrame) -> EnergyDistanceResult:
        return EnergyDistanceResult(
