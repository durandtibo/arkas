...
from arkas.result import MultilabelRecallResult, Result
...
class MultilabelRecallEvaluator(BaseLazyEvaluator[MultilabelRecallResult]):
    ...
    def evaluate(self, data: pl.DataFrame, lazy: bool = True) -> MultilabelRecallResult | Result:
        logger.info(
            f"Evaluating the multilabel recall | y_true={self._y_true} | "
            f"y_pred={self._y_pred} | drop_nulls={self._drop_nulls}"
        )
        return self._evaluate(data, lazy)

    def _compute_result(self, data: pl.DataFrame) -> MultilabelRecallResult:
