...
def test_multilabel_precision_evaluator_evaluate_nan_policy(nan_policy: str) -> None:
    assert (
        MultilabelPrecisionEvaluator(y_true="target", y_pred="pred", nan_policy=nan_policy)
        .evaluate(
            pl.DataFrame(
                {
                    "pred": [
