...
def test_mean_absolute_error_evaluator_evaluate_nan_policy(nan_policy: str) -> None:
    assert (
        MeanAbsoluteErrorEvaluator(y_true="target", y_pred="pred", nan_policy=nan_policy)
        .evaluate(
            pl.DataFrame(
                {
