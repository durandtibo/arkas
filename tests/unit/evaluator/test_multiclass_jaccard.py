...
def test_multiclass_jaccard_evaluator_evaluate_nan_policy(nan_policy: str) -> None:
    assert (
        MulticlassJaccardEvaluator(y_true="target", y_pred="pred", nan_policy=nan_policy)
        .evaluate(
            pl.DataFrame(
                {
                    "pred": [3, 2, 0, 1, 0, None, 1, None],
